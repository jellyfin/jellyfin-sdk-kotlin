...
public class Main {
    public static void main(String[] args) {
        ...
        var jellyfin = new Jellyfin(options);

        // TODO Replace below with a proper CLI that compares to the kotlin-cli sample
        var logger = LoggerFactory.getLogger(Main.class);
        var client = jellyfin.createApi("https://demo.jellyfin.org/stable/");

        var latch = new CountDownLatch(1);
        var userApi = new UserApi(client);
        userApi.authenticateUserByName(new AuthenticateUserByName("demo", null, ""), new JavaDataCallback<>() {
            @Override
