...
public class StreamBuilder
{
	...
	private java.util.ArrayList<PlayMethod> GetAudioDirectPlayMethods(MediaSourceInfo item, MediaStream audioStream, AudioOptions options)
	{
		...
		else
		{
			...
			_logger.Info("Profile: %s, No direct play profiles found for Path: %s", (tempVar != null) ? tempVar : "Unknown Profile", (tempVar2 != null) ? tempVar2 : "Unknown path");
		}
	...
	private StreamInfo BuildVideoItem(MediaSourceInfo item, VideoOptions options)
	{
		...
		_logger.Info("Profile: %s, Path: %s, isEligibleForDirectPlay: %b, isEligibleForDirectStream: %b", (tempVar4 != null) ? tempVar4 : "Unknown Profile", (tempVar5 != null) ? tempVar5 : "Unknown path", isEligibleForDirectPlay, isEligibleForDirectStream);
	...
	private PlayMethod GetVideoDirectPlayProfile(DeviceProfile profile, MediaSourceInfo mediaSource, MediaStream videoStream, MediaStream audioStream, boolean isEligibleForDirectPlay, boolean isEligibleForDirectStream)
	{
		if (videoStream == null)
		{
			...
			_logger.Info("Profile: %s, Cannot direct stream with no known video stream. Path: %s", (tempVar != null) ? tempVar : "Unknown Profile", (tempVar2 != null) ? tempVar2 : "Unknown path");
		...
		if (directPlay == null)
		{
			...
			_logger.Info("Profile: %s, No direct play profiles found for Path: %s", (tempVar3 != null) ? tempVar3 : "Unknown Profile", (tempVar4 != null) ? tempVar4 : "Unknown path");
		...
		if (tangible.DotNetToJavaStringHelper.isNullOrEmpty(videoCodec))
		{
			...
			_logger.Info("Profile: %s, DirectPlay=false. Reason=Unknown video codec. Path: %s", (tempVar6 != null) ? tempVar6 : "Unknown Profile", (tempVar7 != null) ? tempVar7 : "Unknown path");
		...
		if (audioStream != null)
		{
			...
			if (tangible.DotNetToJavaStringHelper.isNullOrEmpty(audioCodec))
			{
				...
				_logger.Info("Profile: %s, DirectPlay=false. Reason=Unknown audio codec. Path: %s", (tempVar8 != null) ? tempVar8 : "Unknown Profile", (tempVar9 != null) ? tempVar9 : "Unknown path");
	...
	private void LogConditionFailure(DeviceProfile profile, String type, ProfileCondition condition, MediaSourceInfo mediaSource)
	{
		...
		_logger.Info("Profile: %s, DirectPlay=false. Reason=%s.%s Condition: %s. ConditionValue: %s. IsRequired: %b. Path: %s", type, (tempVar != null) ? tempVar : "Unknown Profile", condition.getProperty(), condition.getCondition(), (tempVar2 != null) ? tempVar2 : "", condition.getIsRequired(), (tempVar3 != null) ? tempVar3 : "Unknown path");
	}
	...
	private boolean IsEligibleForDirectPlay(MediaSourceInfo item, Integer maxBitrate, MediaStream subtitleStream, VideoOptions options, PlayMethod playMethod)
	{
		if (subtitleStream != null)
		{
			...
			{
				_logger.Info("Not eligible for %s due to unsupported subtitles", playMethod);
				return false;
			}
		}
